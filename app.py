...
def run_pipeline(input_path: str, patient_id: str) -> str:
    ...
    try:
        core.insert_test_results_http(API_BASE, patient_id, all_test_results, exam_date)
    ...
    results_buffers = core.create_detailed_results_page(page_width, page_height, display_data, exam_date, comparative=bool(comparative_data), session_count=session_count)
...
def main():
    ...
    with st.container():
        ...
        with col2:
            st.markdown("### Notes")
            st.write("- Uses `header.png`, `footer.png`, `coverpage.png`, `full_page_with_boxes.png` for branding.")
            st.write("- Fetches Gemini API key from Cloudflare automatically.")
            st.write("- Stores and retrieves test history via admin.fitcarvaan.com API.")
